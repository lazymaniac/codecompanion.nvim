...
T["ReasoningVisualizer"]["auto_visualize"] = function()
  -- Test chain detection
  local chain = { steps = { { content = "test", reasoning = "test reasoning" } } }
  local result = ReasoningVisualizer.auto_visualize(chain)
  h.expect_contains("Chain of Thoughts", result)

  -- Test tree detection
  local tree = { content = "root", children = { { content = "child" } } }
  result = ReasoningVisualizer.auto_visualize(tree)
  h.expect_contains("Tree of Thoughts", result)

  -- Test graph detection
  local graph = { nodes = { n1 = { content = "node" } }, edges = {} }
  result = ReasoningVisualizer.auto_visualize(graph)
  h.expect_contains("Graph of Thoughts", result)

  -- Test unknown structure
  local unknown = { some_field = "value" }
  result = ReasoningVisualizer.auto_visualize(unknown)
  h.expect_contains("Unknown reasoning structure", result)
end

T["ReasoningVisualizer"]["create_summary"] = function()
  -- Test chain summary
  local chain = {
    steps = {
      { completed = true },
  ...
  local result = ReasoningVisualizer.create_summary(chain)
  h.expect_contains("2/3 steps completed", result)

  -- Test graph summary
  local graph = {
    nodes = { n1 = {}, n2 = {}, n3 = {} },
    completed_nodes = { n1 = true },
    failed_nodes = { n2 = true },
  }
  result = ReasoningVisualizer.create_summary(graph)
  h.expect_contains("1 completed, 1 failed, 3 total", result)
end
