...
## Available Actions:
- **add_node**: Add new reasoning nodes to the graph
- **add_edge**: Create dependencies between nodes
- **get_best_path**: Find optimal path between two nodes
- **evaluate_node**: Score and evaluate a specific node
- **view_graph**: Display the complete graph structure
- **merge_nodes**: Combine multiple nodes into one
- **suggest_merges**: Get merge suggestions
- **get_mergeable_nodes**: Find nodes that can be merged
...
function Actions.add_node(args, agent_state)
  ...
  log:debug("[Graph of Thoughts Engine] Adding node: %s", args.content)

  local node_id = agent_state.current_instance:add_node(args.content, args.id)
...
function Actions.add_edge(args, agent_state)
  ...
  if agent_state.current_instance:has_cycle() then
    agent_state.current_instance:remove_edge(args.source_id, args.target_id)
...
function GraphOfThoughtEngine.get_config()
  return {
    ...
    description = "Graph of Thoughts reasoning agent that systematically manages complex interconnected reasoning networks for comprehensive problem-solving.",
    actions = Actions,
    validation_rules = {
      initialize = { "goal" },
      add_node = { "content" },
      add_edge = { "source_id", "target_id" },
      get_best_path = { "start_id", "end_id" },
